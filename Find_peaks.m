...
figure,
ID = 1;
plot(H_patients(ID).tN1,H_patients(ID).Norm1+0.58,'LineWidth',3)
[pks,locs,w,p] = findpeaks(H_patients(ID).Norm1+0.58,H_patients(ID).tN1);
hold on
scatter(locs,pks,100,'ro','filled')
% axis tight
xlabel("Time (s)")
ylabel("Flux (mT)")
...
% figure,
% dt = 1/Fs;
% N = length(C_patients(ID).Norm1);
% t = C_patients(ID).tN1;
% Y = fft(C_patients(ID).Norm1,N);  % computes the (fast) discrete fourier transform
% PSD = abs(Y.*conj(Y)/N);  % Power spectrum (how much power in each freq)
% freq = 1/(dt*N)*(0:N);  %create the x-axis of frequencies in Hz
% L = 1:floor(N/2);  % only plot the first half of freqs
% figure,
% plot(freq(L),PSD(L),'b','LineWidth',1.5)
...
for ID = 1:length(H_patients) 
    if ~isempty(H_patients(ID).Hold) && ID ~= 15 && ID ~= 28 && ID ~= 75 && ...
            ID ~= 71 && ID ~= 70 && ID ~= 68 && ID ~= 57 && ID ~= 29 && ...
            ID ~= 112 && ID ~= 101
        if ~isnan(H_patients(ID).Hold(1))
            ...
            idxt_H(ID) = find(H_patients(ID).tH == locs(idx(end)+1));
            Flux_mean_holdH(ID) = mean(H_patients(ID).Hold(1:idxt_H(ID)));
            Flux_std_holdH(ID) = std(H_patients(ID).Hold(1:idxt_H(ID)));
%             figure,
%             subplot(2,1,1)
%             plot(H_patients(ID).tH,H_patients(ID).Hold)
%             hold on
%             scatter(locs,pks,'r','filled')
%             plot(H_patients(ID).tH(1:idxt_H(ID)),H_patients(ID).Hold(1:idxt_H(ID)),'r')
%             xlabel('Time')
%             ylabel('Normalized Flux')
...
idxt_H(length(H_patients)) = 0;
idx_H(length(H_patients)) = 0;
...
thr = 0.3;
for i = 1:length(H_patients) 
    if i == 15 || i == 28 || i == 75 || i == 71 || i == 70 || i == 68 ||...
            i == 57 || i == 29 || i == 112 || i == 101
        continue
    end
    if idxt_H(i) == 0
        idxt_H(i) = 1;
    end
    signal = H_patients(i).Hold(idxt_H(i):end);
    if isempty(signal) 
        continue
    end
...
for i = 1:length(C_patients)
    ...
    for j = 1:N_epoch
        ...
        plot(y(:,1),y(:,2),'o-','LineWidth',1,'MarkerSize',2);
            
%         title('Signal state space reconstruction','FontSize',18);
        axis off
        grid on
%         set(gca,'CameraPosition',[25.919 27.36 13.854]);
        xlabel('x(t)','FontSize',18,'FontWeight','bold');
        ylabel('x(t+\tau)','FontSize',18,'FontWeight','bold');
        zlabel('x(t+2\tau)','FontSize',18,'FontWeight','bold');
%         axis tight
...
for i = 1:length(H_patients)
    ...
    if ~isempty(H_patients(i).Hold)
        if ~isnan(H_patients(i).Hold(1))
            ...
            tmp = 1./diff(locs);
            peak_stat_H(i,9) = mean(tmp(idx_H(i):end));
            peak_stat_H(i,10) = std(tmp(idx_H(i):end));

            peak_stat_H(i,11) = mean(p(idx_H(i):end));
            peak_stat_H(i,12) = std(p(idx_H(i):end));
        end
    end
end
...
X_H = [peak_stat_H(find(peak_stat_H(:,11)~=0),11) peak_stat_H(find(peak_stat_H(:,12)~=0),12)...
    sumP_holdH' idxt_H(find(idxt_H~=1&idxt_H~=0))' Flux_mean_holdH' Flux_std_holdH'...
    medfreq_holdH' peak2peak_H' rssq_H' rqa_statH];
X_C = [peak_stat_C(find(peak_stat_C(:,11)~=0),11) peak_stat_C(find(peak_stat_C(:,12)~=0),12)...
    sumP_holdC' idxt_C(find(idxt_C~=1))' Flux_mean_holdC' Flux_std_holdC'...
    medfreq_holdC' peak2peak_C' rssq_C' rqa_statC];
% [recrate DET LMAX ENT TND LAM TT]
X_H1 = X_H([find(demo.Age(find(peak_stat_H(1:100,11)~=0))>=18); [54:58]'],:);
...
% RRV_H = [NPSD' SDSD' SDNN' RMSSD' TRI' TINN' DFA' CD' ApEn' pLF' pHF' LFHFratio' VLF' SD1' SD2' SD1SD2ratio'];
% RRV_C = [NPSD_C' SDSD_C' SDNN_C' RMSSD_C' TRI_C' TINN_C' DFA_C' CD_C' ApEn_C' pLF_C' pHF_C' ...
%     LFHFratio_C' VLF_C' SD1_C' SD2_C' SD1SD2ratio_C'];
% 
% ks2stat = zeros(1,size(RRV_H,2));
% for i = 1:length(ks2stat)
%     [~,~,ks2stat(i)] = kstest2(RRV_H(:,i),RRV_C(:,i));
% end
% cat = categorical({'NPSD','SDSD','SDNN',...
%     'RMSSD','TRI','TINN','DFA','CD','ApEn'...
%     'pLF', 'pHF', 'LFHFratio' ,'VLF', 'SD1', 'SD2', 'SD1SD2ratio'});
% bar(cat,ks2stat)
% xlabel('Features')
% ylabel('KS statistic')
% % Isomap
% load('R:\Research\Res_sensor_paper\X_isomap.mat')
% figure,
% scatter3(mX(1:length(X_H),1),mX(1:length(X_H),2),mX(1:length(X_H),3),20,'filled')
% hold on
% scatter3(mX(length(X_H)+1:end,1),mX(length(X_H)+1:end,2),mX(length(X_H)+1:end,3),20,'filled')
...
% Laplacian
% load('R:\Research\Res_sensor_paper\X_Laplacian.mat')
% figure,
% scatter3(mX(1:length(X_H),1),mX(1:length(X_H),2),mX(1:length(X_H),3),20,'filled')
% hold on
% scatter3(mX(length(X_H)+1:end,1),mX(length(X_H)+1:end,2),mX(length(X_H)+1:end,3),20,'filled')
...
% Diffusion map
% load('R:\Research\Res_sensor_paper\X_DiffusionMap.mat')
% figure,
% scatter3(mX(1:length(X_H),1),mX(1:length(X_H),2),mX(1:length(X_H),3),20,'filled')
% hold on
% scatter3(mX(length(X_H)+1:end,1),mX(length(X_H)+1:end,2),mX(length(X_H)+1:end,3),20,'filled')
...
% 
% % tSNE
% load('R:\Research\Res_sensor_paper\X_tSNE.mat')
% figure,
% scatter3(mX(1:length(X_H),1),mX(1:length(X_H),2),mX(1:length(X_H),3),20,'filled')
% hold on
% scatter3(mX(length(X_H)+1:end,1),mX(length(X_H)+1:end,2),mX(length(X_H)+1:end,3),20,'filled')
...
% % Autoencoder
% load('R:\Research\Res_sensor_paper\X_AutoEncoder.mat')
% figure,
% scatter3(mX(1:length(X_H),1),mX(1:length(X_H),2),mX(1:length(X_H),3),20,'filled')
% hold on
% scatter3(mX(length(X_H)+1:end,1),mX(length(X_H)+1:end,2),mX(length(X_H)+1:end,3),20,'filled')
