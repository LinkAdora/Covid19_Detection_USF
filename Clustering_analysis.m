...
% Plot data
X1 = table2array(demo(:,[4 8]));
tmp = demo.Gender == 'M';
X = [X1 tmp];
figure,
scatter(X(3,1),X(3,2),20,X(3,3),'filled')
hold on
scatter(X(:,1),X(:,2),20,X(:,3),'filled')
xlabel('Age'),ylabel('BMI')
title('Age and BMI by gender')
legend('Male','Female')
...
% 
% figure,
% plot(sort(sumP_normH))
% hold on
% plot(sort(sumP_normC))
% xlabel('Index')
% ylabel('Power')
% legend('normH','normC')
% 
% figure,
% plot(sort(sumP_holdH))
% hold on
% plot(sort(sumP_holdC))
% legend('holdH','holdC')
% xlabel('Index')
% ylabel('Power')