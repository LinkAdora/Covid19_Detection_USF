...
for i = 1:length(H_patients)
    ...
    if ~isempty(H_patients(i).Hold)
        if ~isnan(H_patients(i).Hold(1))
            ...
            Flux_mean_holdH(i) = mean(H_patients(i).Hold(iptH(count,3):iptH(count,4)));
            Flux_std_holdH(i) = std(H_patients(i).Hold(iptH(count,3):iptH(count,4)));
        end
    end
end
...
for i = 1:length(C_patients)
    ...
    if ~isempty(C_patients(i).Hold)
        if ~isnan(C_patients(i).Hold(1))
            ...
            Flux_mean_holdC(i) = mean(C_patients(i).Hold(iptC(count,3):iptC(count,4)));
            Flux_std_holdC(i) = std(C_patients(i).Hold(iptC(count,3):iptC(count,4)));
        end
    end
end
%% Extract features
...
for i = 1:length(H_patients) 
    ...
    signal = H_patients(i).Hold(iptH(count,3):iptH(count,4));
...
for i = 1:length(H_patients)
    ...
    if ~isempty(H_patients(i).Hold)
        if ~isnan(H_patients(i).Hold(1))
            ...
            end_idx = find(locs >= t(iptH(count,4)));
            end_idx = end_idx(1);
...
for i = 1:length(C_patients)
    ...
    if ~isempty(C_patients(i).Hold)
        if ~isnan(C_patients(i).Hold(1))
            ...
            start_idx = start_idx(1);
            end_idx = find(locs >= t(iptC(count,4)));
            end_idx = end_idx(1);
