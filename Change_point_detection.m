...
for i = 1:size(res_data,2)
    ...
    if ~isempty(tmp)
%         [b,a] = butter(F_order,[fl fu]/(Fs/2),'bandpass');
...
for ID = 65:65%length(H_patients) 
    if ~isempty(H_patients(ID).Deep)
        if ~isnan(H_patients(ID).Deep(1))
            close all, clc
            max_p = 1;
            signal = H_patients(ID).Deep;
            figure,
            findchangepts(signal,'MaxNumChanges',max_p,'Statistic','mean')
            ipt1 = findchangepts(signal,'MaxNumChanges',max_p,'Statistic','mean');
            hold on
            plot(H_patients(ID).Deep(1:ipt1),'b','LineWidth',1.5)
            plot(ipt1:length(signal),H_patients(ID).Deep(ipt1:end),'r','LineWidth',1.5)
            xticks([0 147 293 438 584 729])
            t = H_patients(ID).tD;
            xticklabels({'0',num2str(floor(t(147))),num2str(floor(t(293))),...
                num2str(floor(t(438))),num2str(floor(t(584))),num2str(floor(t(729)))})
            xlabel("Time (s)")
            title('')
            ylabel("Flux (mT)")
            ...
            xlim([0 750])
%             figure,
%             findchangepts(signal,'MaxNumChanges',max_p,'Statistic','std')
%             ipt2 = findchangepts(signal,'MaxNumChanges',max_p,'Statistic','std');
%             figure,
%             findchangepts(signal,'MaxNumChanges',max_p,'Statistic','linear')
%             ipt3 = findchangepts(signal,'MaxNumChanges',max_p,'Statistic','linear');
%             figure,
%             findchangepts(signal,'MaxNumChanges',max_p,'Statistic','rms')
%             ipt4 = findchangepts(signal,'MaxNumChanges',max_p,'Statistic','rms');
            
%             ipt = [ipt1];
%             ipt = [1 151 236];
%             ipt(4) = [];
%             hold_dur = (ipt(2)-ipt(1))/Fs
%             trans_dur = (ipt(3)-ipt(2))/Fs
%             norm_dur = (length(H_patients(ID).Hold)-ipt(3))/Fs
%             T = 1/Fs;             % Sampling period       
%             Y = fft(H_patients(ID).Hold);
%             L = length(H_patients(ID).Hold);             % Length of signal
...
% for ID = 23:23%length(C_patients) 
%     if ~isempty(C_patients(ID).Deep)
%         if ~isnan(C_patients(ID).Deep(1))
%             close all
%             signal = C_patients(ID).Deep;
%             max_p = 3;
%             figure,
%             findchangepts(signal,'MaxNumChanges',max_p,'Statistic','mean')
%             ipt1 = findchangepts(signal,'MaxNumChanges',max_p,'Statistic','mean');
