...
#read in data
dataset = read.spss("R:\\Research\\Res_sensor_paper\\causal_data_normalB.sav", to.data.frame=TRUE)
#view data
# View(dataset)
...
y_trt<-matched$ENT[matched$COVID_state==1]
y_con<-matched$ENT[matched$COVID_state==0]
...
##########################
library(openxlsx)
write.xlsx(matched, 'R:/Research/Res_sensor_paper/matching_normal.xlsx')
