...
bad_HpatientID = [15 21 22 28 35 38 54 59 60 67 68 70:75 101 111 112 108:112];
young_idx1 = find(demo.Age < 18)';
young_idx = [];
...
for i = 1:100
    ...
    if ~ismember(i,bad_HpatientID) && ~isempty(tmp) && ~isnan(tmp(1))  
        ...
        t_before = iptH(i,2):iptH(i,3)-1;
        t_after = iptH(i,3):length(t);
        t_after30 = iptH(i,3):iptH(i,3)+ceil(Fs*30);
        
        sig_before = tmp(t_before);
        sig_after = tmp(t_after);
        sig_after30 = tmp(t_after30);

        [pks,locs,w,p] = findpeaks(sig_after,t(t_after));
        [pks30,locs30,w30,p30] = findpeaks(sig_after30,t(t_after30));

        pp = (p-mean(p))/std(p);
        bad_peaks = find(pp < thresh);
        pks(bad_peaks) = [];
        locs(bad_peaks) = [];

        peak_statH(end+1,:) = [60/mean(diff(locs)) 60/std(diff(locs)) mean(p) std(p) mean(w) std(w),...
                                60/mean(diff(locs30)) 60/std(diff(locs30)) mean(p30) std(p30) mean(w30) std(w30)];
        TfeaH(end+1,:) = [mean(H_patientsN(i).Hold(t_after)) std(H_patientsN(i).Hold(t_after)) peak2peak(sig_after) rssq(sig_after),...
                           mean(H_patientsN(i).Hold(t_after30)) std(H_patientsN(i).Hold(t_after30)) peak2peak(sig_after30) rssq(sig_after30),...
                           mean(H_patientsN(i).Hold(t_before)) std(H_patientsN(i).Hold(t_before)) peak2peak(sig_before) rssq(sig_before)];
...
for i = 1:100
    ...
    if ~ismember(i,bad_HpatientID) && ~isempty(tmp) && ~isnan(tmp(1))   
        ...
        PSD_before = abs(Y.*conj(Y)/L);  % Power spectrum (how much power in each freq)
        L1 = 1:ceil(L/2);  % only plot the first half of freqs
        % extract band power
        idx = find(f1 >= 0.1 & f1 <= 0.5);
        Pband_before = (f1(2)-f1(1))*sum(PSD_before(idx));
        [max_PSDp2,idx2] = max(PSD_before(21:length(f1)));
        domf_before = f1(idx2+20);

        X = sig_after;
        L = length(X);
        f1 = Fs*(0:(L/2))/L;
        Y = fft(X,L);  % computes the (fast) discrete fourier transform
        PSD_after = abs(Y.*conj(Y)/L);  % Power spectrum (how much power in each freq)
        L1 = 1:ceil(L/2);  % only plot the first half of freqs
        % extract band power
        idx = find(f1 >= 0.1 & f1 <= 0.5);
        Pband_after = (f1(2)-f1(1))*sum(PSD_after(idx));
        [max_PSDp2,idx2] = max(PSD_after(21:length(f1)));
        domf_after = f1(idx2+20);

        X = sig_after30;
        L = length(X);
        f1 = Fs*(0:(L/2))/L;
        Y = fft(X,L);  % computes the (fast) discrete fourier transform
        PSD_after30 = abs(Y.*conj(Y)/L);  % Power spectrum (how much power in each freq)
        L1 = 1:ceil(L/2);  % only plot the first half of freqs
        % extract band power
        idx = find(f1 >= 0.1 & f1 <= 0.5);
        Pband_after30 = (f1(2)-f1(1))*sum(PSD_after30(idx));
        [max_PSDp2,idx2] = max(PSD_after30(21:length(f1)));
        domf_after30 = f1(idx2+20);

        X = tmp;
        L = length(X);
        f1 = Fs*(0:(L/2))/L;
        Y = fft(X,L);  % computes the (fast) discrete fourier transform
        PSD_all = abs(Y.*conj(Y)/L);  % Power spectrum (how much power in each freq)
        L1 = 1:ceil(L/2);  % only plot the first half of freqs
        % extract band power
        idx = find(f1 >= 0.1 & f1 <= 0.5);
        Pband_all = (f1(2)-f1(1))*sum(PSD_all(idx));
        [max_PSDp2,idx2] = max(PSD_all(21:length(f1)));
        domf_all = f1(idx2+20);

        FfeaH(end+1,:) = [Pband_before mean(PSD_before) RMS1(PSD_before) meanfreq(sig_before) domf_before,...
                        Pband_after mean(PSD_after) RMS1(PSD_after) meanfreq(sig_after) domf_after,...
                        Pband_after30 mean(PSD_after30) RMS1(PSD_after30) meanfreq(sig_after30) domf_after30,...
                        Pband_all mean(PSD_all) RMS1(PSD_all) meanfreq(tmp) domf_all];
    end
end
%% RQA for healthy
tau = 2;
...
for i = 1:100
    ...
    if ~ismember(i,bad_HpatientID) && ~isempty(tmp) && ~isnan(tmp(1))
        ...
        recurrpt_after = tdrecurr_y(recurdata,thr);

        X = tmp;
        y = phasespace(X,dim,tau);
        recurdata = cerecurr_y(y);
        recurrpt_all = tdrecurr_y(recurdata,thr);

        RQA_statH(end+1,:) = [recurrqa_y(recurrpt_after) recurrqa_y(recurrpt_all)];
    end
end
%% Peak analysis for COVID-19
bad_CpatientID = [3 6 17];
thresh = -1.5;
M_size = 10;
peak_statC = [];
TfeaC = [];
for i = 1:length(C_patients)
    tmp = C_patients(i).Hold;
    t = C_patients(i).tH; 
    if ~ismember(i,bad_CpatientID) && ~isempty(tmp) && ~isnan(tmp(1))     
        t_before = iptC(i,2):iptC(i,3)-1;
        t_after = iptC(i,3):length(t);
        t_after30 = iptC(i,3):iptC(i,3)+ceil(Fs*30);
        ...
        peak_statC(end+1,:) = [60/mean(diff(locs)) 60/std(diff(locs)) mean(p) std(p) mean(w) std(w),...
                                60/mean(diff(locs30)) 60/std(diff(locs30)) mean(p30) std(p30) mean(w30) std(w30)];
        TfeaC(end+1,:) = [mean(C_patientsN(i).Hold(t_after)) std(C_patientsN(i).Hold(t_after)) peak2peak(sig_after) rssq(sig_after),...
                           mean(C_patientsN(i).Hold(t_after30)) std(C_patientsN(i).Hold(t_after30)) peak2peak(sig_after30) rssq(sig_after30),...
                           mean(C_patientsN(i).Hold(t_before)) std(C_patientsN(i).Hold(t_before)) peak2peak(sig_before) rssq(sig_before)];
...
for i = 1:length(C_patients)
    ...
    if ~ismember(i,bad_CpatientID) && ~isempty(tmp) && ~isnan(tmp(1))   
        ...
        FfeaC(end+1,:) = [Pband_before mean(PSD_before) RMS1(PSD_before) meanfreq(sig_before) domf_before,...
                        Pband_after mean(PSD_after) RMS1(PSD_after) meanfreq(sig_after) domf_after,...
                        Pband_after30 mean(PSD_after30) RMS1(PSD_after30) meanfreq(sig_after30) domf_after30,...
                        Pband_all mean(PSD_all) RMS1(PSD_all) meanfreq(tmp) domf_all];
    end
...
for i = 1:length(C_patients)
    ...
    if ~ismember(i,bad_CpatientID) && ~isempty(tmp) && ~isnan(tmp(1))   
        ...
        RQA_statC(end+1,:) = [recurrqa_y(recurrpt_after) recurrqa_y(recurrpt_all)];
    end
end
...
peak_statAll = [peak_statH(:,[3 4 9]); peak_statC(:,[3 4 9])];
TfeaAll = [TfeaH(:,[4 9 10]); TfeaC(:,[4 9 10])];
FfeaAll = [FfeaH(:,[6 14 19]); FfeaC(:,[6 14 19])];
RQA_All = [RQA_statH(:,[1 3 5]); RQA_statC(:,[1 3 5])];
