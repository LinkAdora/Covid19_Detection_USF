...
%% Feature extraction for holing breath for healthy patients no filter
% count = 0;
...
%         % Holding section  
%         meanHoldH(count) = mean(sig(t_hold));
%         medHoldH(count) = median(sig(t_hold));
%         stdHoldH(count) = std(sig(t_hold));
%         skewHoldH(count) = skewness(sig(t_hold));
%         p2pHoldH(count) = peak2peak(sig(t_hold));
%         RSSQHoldH(count) = rssq(sig(t_hold));
%         madHoldH(count) = mad(sig(t_hold)); % mean absolute deviation
% 
%         % After-holding section
%         meanAfterH(count) = mean(sig(t_after));
%         medAfterH(count) = median(sig(t_after));
%         stdAfterH(count) = std(sig(t_after));
%         skewAfterH(count) = skewness(sig(t_after));
%         p2pAfterH(count) = peak2peak(sig(t_after));
%         RSSQAfterH(count) = rssq(sig(t_after));
%         madAfterH(count) = mad(sig(t_after)); % mean absolute deviation
%     end
% end
% varNames = ["ID","meanHold","medHold","stdHold","skewHold","p2pHold","RSSQHold","madHold",...
%     "meanAfter","medAfter","stdAfter","skewAfter","p2pAfter","RSSQAfter","madAfter"];
% F_holdH = table(IDH',meanHoldH',medHoldH',stdHoldH',skewHoldH',p2pHoldH',RSSQHoldH',madHoldH',...
%     meanAfterH',medAfterH',stdAfterH',skewAfterH',p2pAfterH',RSSQAfterH',madAfterH','VariableNames',varNames);
%% Feature extraction for holding breath for COVID patients no filter
% count = 0;
...
%         % Holding section
%         meanHoldC(count) = mean(sig(t_hold));
%         medHoldC(count) = median(sig(t_hold));
%         stdHoldC(count) = std(sig(t_hold));
%         skewHoldC(count) = skewness(sig(t_hold));
%         p2pHoldC(count) = peak2peak(sig(t_hold));
%         RSSQHoldC(count) = rssq(sig(t_hold));
%         madHoldC(count) = mad(sig(t_hold)); % mean absolute deviation
% 
%         % After-holding section
%         meanAfterC(count) = mean(sig(t_after));
%         medAfterC(count) = median(sig(t_after));
%         stdAfterC(count) = std(sig(t_after));
%         skewAfterC(count) = skewness(sig(t_after));
%         p2pAfterC(count) = peak2peak(sig(t_after));
%         RSSQAfterC(count) = rssq(sig(t_after));
%         madAfterC(count) = mad(sig(t_after)); % mean absolute deviation
%     end
% end
% varNames = ["ID","meanHold","medHold","stdHold","skewHold","p2pHold","RSSQHold","madHold",...
%     "meanAfter","medAfter","stdAfter","skewAfter","p2pAfter","RSSQAfter","madAfter"];
% F_holdC = table(IDC',meanHoldC',medHoldC',stdHoldC',skewHoldC',p2pHoldC',RSSQHoldC',madHoldC',...
%     meanAfterC',medAfterC',stdAfterC',skewAfterC',p2pAfterC',RSSQAfterC',madAfterC','VariableNames',varNames);
%% Feature extraction for holing breath for healthy patients with filter
count = 0;
for i = 8:length(H_patients)
    if ~isnan(iptH(i,2))
        count = count + 1;
        IDH(count) = i;
        tH = H_patients(i).tH;
        ...
        RSSQHoldH(count) = rssq(sig(t_hold));
        madHoldH(count) = mad(sig(t_hold)); % mean absolute deviation
        holdDurH(count) = length(t_hold)/Fs;
        % After-holding section
        meanAfterH(count) = mean(sig2);
        medAfterH(count) = median(sig2);
        stdAfterH(count) = std(sig2);
        skewAfterH(count) = skewness(sig2);
        p2pAfterH(count) = peak2peak(sig2);
        RSSQAfterH(count) = rssq(sig2);
        madAfterH(count) = mad(sig2); % mean absolute deviation
        close all
    end
end
varNames = ["ID","meanHold","medHold","stdHold","skewHold","p2pHold","RSSQHold","madHold","holdDurH"...
    "meanAfter","medAfter","stdAfter","skewAfter","p2pAfter","RSSQAfter","madAfter"];
F_holdH = table(IDH',meanHoldH',medHoldH',stdHoldH',skewHoldH',p2pHoldH',RSSQHoldH',madHoldH',holdDurH',...
    meanAfterH',medAfterH',stdAfterH',skewAfterH',p2pAfterH',RSSQAfterH',madAfterH','VariableNames',varNames);
...
for i = 1:length(C_patients)
    if ~isnan(iptC(i,2))
        ...
        RSSQHoldC(count) = rssq(sig(t_hold));
        madHoldC(count) = mad(sig(t_hold)); % mean absolute deviation
        holdDurC(count) = length(t_hold)/Fs;
        % After-holding section
        meanAfterC(count) = mean(sig2);
        medAfterC(count) = median(sig2);
        stdAfterC(count) = std(sig2);
        skewAfterC(count) = skewness(sig2);
        p2pAfterC(count) = peak2peak(sig2);
        RSSQAfterC(count) = rssq(sig2);
        madAfterC(count) = mad(sig2); % mean absolute deviation
        close all
    end
end
varNames = ["ID","meanHold","medHold","stdHold","skewHold","p2pHold","RSSQHold","madHold","holdDurC"...
    "meanAfter","medAfter","stdAfter","skewAfter","p2pAfter","RSSQAfter","madAfter"];
F_holdC = table(IDC',meanHoldC',medHoldC',stdHoldC',skewHoldC',p2pHoldC',RSSQHoldC',madHoldC',holdDurC',...
    meanAfterC',medAfterC',stdAfterC',skewAfterC',p2pAfterC',RSSQAfterC',madAfterC','VariableNames',varNames);
%% Plot histogram of KS-stat
X_H = table2array(F_holdH(:,2:end));
X_C = table2array(F_holdC(:,2:end));
X = [X_H; X_C];
...
cat = categorical({'meanHold','medHold','stdHold','skewHold','p2pHold','RSSQHold','madHold','holdDur',...
    'meanAfter','medAfter','stdAfter','skewAfter','p2pAfter','RSSQAfter','madAfter'});
bar(cat,ks2stat)
xlabel('Features')
ylabel('KS statistic')