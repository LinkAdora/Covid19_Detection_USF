...
for i = 1:length(H_patients)
    if ~isnan(iptH(i,2))
        ...
        N = length(X);
        Y = fft(X,N);  % computes the (fast) discrete fourier transform
        PSD = abs(Y.*conj(Y)/N);  % Power spectrum (how much power in each freq)
        L = 1:floor(N/2);  % only plot the first half of freqs
        % Extract features
        medFreqH(count) = medfreq(X,Fs);
        meanFreqH(count) = meanfreq(X,Fs);
        [~,idx] = max(PSD);
        domFreqH(count) = f(idx);
        meanPSDH(count) = mean(PSD);
        meanP1H(count) = mean(P1);
        medPSDH(count) = median(PSD);
        medP1H(count) = median(P1);
        NPSDH(count) = RMS1(PSD);
        NP1H(count) = RMS1(P1);
        sumPH(count) = sum(P1(find(f >= 0.05 | f <= 0.4)));
    end
end
F_holdH = table(IDH',medFreqH',meanFreqH',domFreqH',meanPSDH',meanP1H',medPSDH',medP1H',NPSDH',...
    NP1H',sumPH');
...
for i = 1:length(C_patients)
    if ~isnan(iptC(i,2))
        ...
        PSD = abs(Y.*conj(Y)/N);  % Power spectrum (how much power in each freq)
        L = 1:floor(N/2);  % only plot the first half of freqs
        sumPC(count) = sum(P1(find(f >= 0.05 | f <= 0.4)));
        % Extract features
        medFreqC(count) = medfreq(X,Fs);
        meanFreqC(count) = meanfreq(X,Fs);
        [~,idx] = max(PSD);
        domFreqC(count) = f(idx);
        meanPSDC(count) = mean(PSD);
        meanP1C(count) = mean(P1);
        medPSDC(count) = median(PSD);
        medP1C(count) = median(P1);
        NPSDC(count) = RMS1(PSD);
        NP1C(count) = RMS1(P1);
        sumPC(count) = sum(P1(find(f >= 0.05 | f <= 0.4)));
    end
end
F_holdC = table(IDC',medFreqC',meanFreqC',domFreqC',meanPSDC',meanP1C',medPSDC',medP1C',NPSDC',...
    NP1C',sumPC');
...
cat = categorical({'medFreq','meanFreq','domFreq','meanPSD','meanP1','medPSD','medP1','NPSD',...
    'NP1','sumP'});
bar(cat,ks2stat)
xlabel('Features')
ylabel('KS statistic')