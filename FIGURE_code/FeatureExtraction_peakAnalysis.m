...
for i = 1:length(H_patients)
    if ~isnan(iptH(i,2))
        ...
        % Hold breathingC
        [~,~,w,p] = findpeaks(sig2,H_patients(i).tH(t_after));
        t = (p-mean(p))/std(p);
        bad_peaks = find(t < -2);
        p(bad_peaks) = [];
        w(bad_peaks) = [];
        % Extract features
        meanPeakPH(count) = mean(p);
        meanPeakWH(count) = mean(w);
        stdPeakPH(count) = std(p);
        stdPeakWH(count) = std(w);
        skewPeakPH(count) = skewness(p);
        skewPeakWH(count) = skewness(w);
        kurPeakPH(count) = kurtosis(p);
        kurPeakWH(count) = kurtosis(w);
    end
end
F_holdH = table(IDH',meanPeakPH',meanPeakWH',stdPeakPH',stdPeakWH',skewPeakPH',...
    skewPeakWH',kurPeakPH',kurPeakWH');
...
for i = 1:length(C_patients)
    if ~isnan(iptC(i,2))
        ...
        % Hold breathing
        [~,~,w,p] = findpeaks(sig2,C_patients(i).tH(t_after));
        t = (p-mean(p))/std(p);
        bad_peaks = find(t < -2);
        p(bad_peaks) = [];
        w(bad_peaks) = [];
        % Extract features
        meanPeakPC(count) = mean(p);
        meanPeakWC(count) = mean(w);
        stdPeakPC(count) = std(p);
        stdPeakWC(count) = std(w);
        skewPeakPC(count) = skewness(p);
        skewPeakWC(count) = skewness(w);
        kurPeakPC(count) = kurtosis(p);
        kurPeakWC(count) = kurtosis(w);
    end
end
F_holdC = table(IDC',meanPeakPC',meanPeakWC',stdPeakPC',stdPeakWC',skewPeakPC',...
    skewPeakWC',kurPeakPC',kurPeakWC');
...
cat = categorical({'meanPeakP','meanPeakW','stdPeakP','stdPeakW','skewPeakPC',...
    'skewPeakWC','kurPeakPC','kurPeakWC'});
bar(cat,ks2stat)
xlabel('Features')
ylabel('KS statistic')