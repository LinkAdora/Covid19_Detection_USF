...
for i = 1:length(H_patients)
    ...
    if ~isempty(H_patients(i).Hold)
        if ~isnan(H_patients(i).Hold(1))
            ...
            Flux_mean_holdH(i) = mean(H_patients(i).Hold(iptH(i,4):end));
            Flux_std_holdH(i) = std(H_patients(i).Hold(iptH(i,4):end));
        end
    end
end
...
for i = 1:length(H_patients) 
    ...
    signal = H_patients(i).Hold(iptH(i,4):end);
...
for i = 1:length(H_patients)
    ...
    if ~isempty(H_patients(i).Hold)
        if ~isnan(H_patients(i).Hold(1))
            ...
            t = H_patients(i).tH;
            tmp = 1./diff(locs);
            start_idx = find(locs >= t(iptH(i,4)));
