...
% bad_HpatientID = [4 15 21 22 24 28 35 36 38 54 57 59 60 65 67 68 70:75 101 111 112 14 17 40 108:112];
...
for i = 1:length(H_patients)
    ...
    if ~isempty(H_patients(i).Hold)
        if ~isnan(iptH(i,3))
            ...
            Flux_mean_holdH(i) = mean(H_patients(i).Hold(iptH(i,3):end));
            Flux_std_holdH(i) = std(H_patients(i).Hold(iptH(i,3):end));
        end
    end
end
...
for i = 1:length(C_patients)
    ...
    if ~isempty(C_patients(i).Hold)
        if ~isnan(C_patients(i).Hold(1)) && ~isnan(iptC(i,2))
            [pks,locs,w,p] = findpeaks(C_patients(i).Hold,C_patients(i).tH);
            Flux_mean_holdC(i) = mean(C_patients(i).Hold(iptC(i,3):end));
            Flux_std_holdC(i) = std(C_patients(i).Hold(iptC(i,3):end));
        end
    end
end
%% Extract features
...
thr = 0.3;
for i = 1:length(H_patients) 
    if ismember(i,bad_HpatientID) || isnan(iptH(i,3))
        continue
    end
    ...
    signal = H_patients(i).Hold(iptH(i,3):end);
...
for i = 1:length(C_patients)
    if ismember(i,bad_CpatientID) || isnan(iptC(i,3))
        continue
    end
    if isempty(C_patients(i).Hold) 
        continue
    ...
    signal = C_patients(i).Hold(iptC(i,3):end);
    if floor(length(signal)/N_eps) ~= 1 && floor(length(signal)/N_eps) ~= 0
        N_epoch = floor(length(signal)/N_eps);
    else
        N_epoch = 1;
    ...
    if ~isempty(H_patients(i).Hold)
        if ~isnan(H_patients(i).Hold(1))
           ...
           start_idx = find(locs >= t(iptH(i,3)));
            ...
            peak_stat_H(i,9) = mean(tmp(start_idx:end));
            peak_stat_H(i,10) = std(tmp(start_idx:end));

            peak_stat_H(i,11) = mean(p(start_idx:end));
            peak_stat_H(i,12) = std(p(start_idx:end));
        end
    end
end
...
for i = 1:length(C_patients)
    ...
    if ~isempty(C_patients(i).Hold)
        if ~isnan(C_patients(i).Hold(1))
            ...
            peak_stat_C(i,9) = mean(tmp(start_idx:end));
            peak_stat_C(i,10) = std(tmp(start_idx:end));

            peak_stat_C(i,11) = mean(p(start_idx:end));
            peak_stat_C(i,12) = std(p(start_idx:end));
        end
    end
end
...
X_H1 = X_H([find(demo.Age(find(peak_stat_H(1:100,11)~=0))>=18); [length(X_H)-3:length(X_H)]'],:);
