...
iptH = table2array(changepointdetectionS2);
...
iptC = table2array(changepointdetectionS3);
clear opts
...
Flux_std_holdH = [];
Flux_std_holdC = [];
bad_HpatientID = [65:74 107 108];
bad_CpatientID = [17 32 33];
count = 0;
for i = 1:length(H_patients)
    ...
    if ~isempty(H_patients(i).Hold)
        if ~isnan(H_patients(i).Hold(1))
            ...
            Flux_mean_holdH(i) = mean(H_patients(i).Hold(iptH(count,4):end));
            Flux_std_holdH(i) = std(H_patients(i).Hold(iptH(count,4):end));
        end
    end
end
...
for i = 1:length(C_patients)
    ...
    if ~isempty(C_patients(i).Hold)
        if ~isnan(C_patients(i).Hold(1))
            ...
            Flux_mean_holdC(i) = mean(C_patients(i).Hold(iptC(i,4):end));
            Flux_std_holdC(i) = std(C_patients(i).Hold(iptC(i,4):end));
        end
    end
end
%% Extract features
...
for i = 1:length(H_patients) 
    ...
    hold_durH(i) = (iptH(count,3)-iptH(count,2))/Fs;
    trans_durH(i) = (iptH(count,4)-iptH(count,3))/Fs;
    signal = H_patients(i).Hold(iptH(count,4):end);
...
for i = 1:length(C_patients)
    ...
    signal = C_patients(i).Hold(iptC(i,4):end);
    if floor(length(signal)/N_eps) ~= 1 && floor(length(signal)/N_eps) ~= 0
        N_epoch = floor(length(signal)/N_eps);
    else
        N_epoch = 1;
...
for i = 1:length(H_patients)
    ...
    if ~isempty(H_patients(i).Hold)
        if ~isnan(H_patients(i).Hold(1))
            ...
            bad_peaks = find(p < 0.01);
            locs(bad_peaks) = [];
            p(bad_peaks) = [];
            t = H_patients(i).tH;
            tmp = 1./diff(locs);
            start_idx = find(locs >= t(iptH(count,4)));
...
for i = 1:length(C_patients)
    ...
    if ~isempty(C_patients(i).Hold)
        if ~isnan(C_patients(i).Hold(1))
            ...
            tmp = 1./diff(locs);
            t = C_patients(i).tH;
            start_idx = find(locs >= t(iptC(i,4)));
            start_idx = start_idx(1);
...
% feas = [1 9];
X = [X_H(:,feas); X_C(:,feas)];
Y = [zeros(size(X_H,1),1); ones(length(X_C),1)];
