...
%% Plot healthy and COVID-19 patients
figure,
ID1 = 1;
ID2 = 1;
ID3 = 67;
% subplot(3,1,1)
plot(H_patients(ID1).tN1,H_patients(ID1).Norm1,'LineWidth',5)
xlim([0 150])
axis tight
ylabel("Flux (mT)")
...
% subplot(3,1,2)
% plot(H_patients(ID2).tH,H_patients(ID2).Hold,'LineWidth',1.5)
% xlim([0 150])
% axis tight
% ylabel("Flux (mT)")
% set(gca,'FontSize',18,'FontWeight','bold')
...
% 
% subplot(3,1,3)
% plot(H_patients(ID3).tD,H_patients(ID3).Deep,'LineWidth',1.5)
% xlim([0 150])
% axis tight
% ylabel("Flux (mT)")
% set(gca,'FontSize',18,'FontWeight','bold')
...
% axis off
% subplot(2,1,2)
% plot(C_patients(ID2).tD(1:tt),C_patients(ID2).Deep(1:tt),'r')
% axis tight
% xlabel("Time (s)")
% ylabel("Flux (mT)")
% xlim([0 150])
...
for ID = 1:length(C_patients)
    if ~isempty(C_patients(ID).Norm1)
        if ~isnan(C_patients(ID).Norm1)
            Flux_mean_holdC(end+1) = mean(C_patients(ID).Norm1);
            Flux_std_holdC(end+1) = std(C_patients(ID).Norm1);
        end
    end
    
    if ~isempty(C_patients(ID).Norm2)
        if ~isnan(C_patients(ID).Norm2)
            Flux_mean_holdC(end+1) = mean(C_patients(ID).Norm2);
            Flux_std_holdC(end+1) = std(C_patients(ID).Norm2);
        end
    end
end
%% Extract features
...
peak_statC_total = [peak_stat_C(find(peak_stat_C(:,1)~=0),1:4); peak_stat_C(find(peak_stat_C(:,5)~=0),5:8)];

X_H = [peak_statH_total...
    sumP_holdH' Flux_std_holdH'...
    medfreq_holdH' peak2peak_H' rssq_H' rqa_statH];
X_C = [peak_statC_total...
    sumP_holdC' Flux_std_holdC'...
...
cat = categorical({'RRmean','RRstd','Peak prominence mean','Peak prominence std','Power','Flux std','Medfreq','peak2peak','rssq'...
